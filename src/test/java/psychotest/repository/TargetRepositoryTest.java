...
public class TargetRepositoryTest {
    ...
    @Autowired
    private TargetRepository studentService;

    @MockBean
    private TargetRepository studentRepository;
    ...
    public void testRetrieveStudentWithMockRepository() throws Exception {
        ...
        when(studentRepository.getDataCall()).thenReturn(list);
        assertTrue(studentService.getDataCall().get(0).getExtidBckgr().contains("2168779357"));
        assertTrue(studentService.getDataCall().get(1).getExtidBckgr().contains("2168779358"));
    }
}